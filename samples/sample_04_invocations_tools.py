"""Sample 03 (variant) - Invocations API with a local tool.
...
before answering.  The Invocations API surfaces only the final
assistant text (and intermediate token deltas when ``"stream": true``);
the tool call/result pair lives inside the graph and is consumed during
the ReAct loop.
...
Run::
    ...
    uv run python samples/sample_03_invocations_tools.py
