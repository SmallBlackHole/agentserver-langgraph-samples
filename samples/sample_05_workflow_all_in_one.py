...
- ``plan`` - LLM call bound to the tools.  Decides whether to call a tool.
  When the model can answer directly (e.g. "Hello") the workflow ends
  here.
...
intermediate ``function_call`` / ``function_call_output`` /
``message`` from the workflow; the Invocations API returns just the
final assistant text (or streams its tokens when ``stream=true``).

Required environment variables (set in ``samples/.env`` or your shell):
...
Run::
    ...
    az login
    cp samples/.env.example samples/.env  # then edit the values
    uv run python samples/sample_05_workflow_all_in_one.py
...
def _build_graph():
    ...
    # tools_condition routes to "tools" when the planner emitted tool_calls,
    # otherwise to END.  We feed the tool results back through synthesize.
