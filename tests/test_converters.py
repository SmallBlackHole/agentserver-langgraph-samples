...
from azure.ai.agentserver.langgraph._converters import (
...
def test_items_to_messages_handles_message_function_call_and_output() -> None:
    items = [
        ItemMessage(role="user", content=[MessageContentInputTextContent({"type": "input_text", "text": "hi"})]),
        ItemFunctionToolCall(call_id="call_1", name="get_weather", arguments='{"city": "Seattle"}'),
