...
async def state_to_events(
    ...
    Walks every message appended after the last :class:`HumanMessage`
    so intermediate tool calls and tool results are surfaced to the
    client (not just the final assistant text).

    :param state: The value returned by ``CompiledStateGraph.ainvoke``.
        Expected shape is a mapping with a ``"messages"`` channel.
    :type state: dict[str, Any] | Any
...
def _messages_for_this_turn(state: Any) -> list[BaseMessage]:
    ...
    :param state: The graph result.
    :type state: Any
    :returns: The list of messages to emit.
...
async def _emit_message(stream: ResponseEventStream, text: str) -> AsyncIterator[dict[str, Any]]:
