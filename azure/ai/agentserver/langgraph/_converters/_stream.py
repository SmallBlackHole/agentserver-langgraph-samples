...
intermediate tool calls and tool-message results to the client in
real time, not only the final assistant message.
...
async def stream_graph_to_events(
    ...
    *,
    ...
    :param graph_stream: The ``CompiledStateGraph.astream`` iterator,
        opened with ``stream_mode=["updates", "messages"]``.
    :type graph_stream: AsyncIterator[Any]
    :param stream: The :class:`ResponseEventStream` to emit events through.
    :type stream: ResponseEventStream
    :param cancellation_signal: Set by the responses host when the
        request is cancelled or the server is draining; iteration stops
        on set.
    :type cancellation_signal: asyncio.Event
    :return: Async iterator of Responses API event payload dicts.
...
class _StreamState:
    ...
    async def handle_message_chunk(self, payload: Any) -> AsyncIterator[dict[str, Any]]:
    ...
    async def handle_update(self, payload: Any) -> AsyncIterator[dict[str, Any]]:
        ...
        the partial state returned by the node, which for ``MessagesState``
        graphs contains a ``messages`` channel with the messages that
        node appended.
        """
    ...
    async def _emit_tool_call(self, call: dict[str, Any]) -> AsyncIterator[dict[str, Any]]:
    ...
    async def _emit_tool_output(self, message: ToolMessage) -> AsyncIterator[dict[str, Any]]:
...
def _split_chunk(chunk: Any) -> tuple[str | None, Any]:
    ...
    :param chunk: One value yielded by ``graph.astream``.
    :type chunk: Any
    :return: A ``(mode, payload)`` pair, with ``mode`` set to ``None``
...
def _extract_node_messages(payload: Any) -> list[list[BaseMessage]]:
    ...
    :param payload: The ``updates`` payload from ``graph.astream``.
    :type payload: Any
    :return: A list of message lists, one per node update found.
    :rtype: list[list[BaseMessage]]
    """
